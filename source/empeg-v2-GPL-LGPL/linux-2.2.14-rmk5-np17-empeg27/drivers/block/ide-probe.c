...
static int try_to_identify (ide_drive_t *drive, byte cmd)
{
	...
	if (IDE_CONTROL_REG) {
		if (!HWIF(drive)->hw.irq) {		/* already got an IRQ? */
	...
	/* On the empeg, we boot very quickly - before the HDD has spun up
	   most of the time. We retry the identifies higher up, and so don't
	   want to wait long here */
	timeout = (HZ/4);

	/* We also want to ensure that no stray capacitance on the bus (as we
	   don't have any pulldowns) is causing false positives on drive
	   scanning. We do this by writing to the IDE status register, which
	   is read-only. */

	/* This seemed to stop detection of drives on some players so
           we don't do it any more. MAC 1999/11/24 */
	/*OUT_BYTE(0,IDE_STATUS_REG);*/
#else
	...
	if (IDE_CONTROL_REG && !HWIF(drive)->hw.irq) {
		...
		if (irqs > 0) {
			HWIF(drive)->hw.irq = irqs; /* save it for later */
			irqs = probe_irq_on();
...
static void probe_hwif (ide_hwif_t *hwif)
{
	unsigned int unit;
	unsigned long flags;
	ide_ioreg_t ide_control_reg = hwif->hw.io_ports[IDE_CONTROL_OFFSET];
	ide_ioreg_t region_low = hwif->hw.io_ports[IDE_DATA_OFFSET];
	...
	for (i = IDE_DATA_OFFSET; i <= IDE_STATUS_OFFSET; i++) {
		if (hwif->hw.io_ports[i]) {
			if (hwif->hw.io_ports[i] < region_low)
				region_low = hwif->hw.io_ports[i];
			if (hwif->hw.io_ports[i] > region_high)
				region_high = hwif->hw.io_ports[i];
		}
	}
...
#endif /* CONFIG_BLK_DEV_PDC4030 */
	 && (ide_check_region(region_low, region_high - region_low) ||
	...
	for (unit = 0; unit < MAX_DRIVES; ++unit) {
		...
		if (drive->present && !hwif->present) {
			hwif->present = 1;
			if (hwif->chipset != ide_4drives || !hwif->mate->present) {
				ide_request_region(region_low, region_high - region_low, hwif->name);
				if (ide_control_reg)
...
static int init_irq (ide_hwif_t *hwif)
{
	...
	if (!match || match->hw.irq != hwif->hw.irq) {
		...
		if (ide_request_irq(hwif->hw.irq, &ide_intr, sa, hwif->name, hwgroup)) {
			if (!match)
				kfree(hwgroup);
	...
	printk("%s at 0x%03x-0x%03x,0x%03x on irq %d", hwif->name,
		...
		(unsigned int)hwif->hw.io_ports[IDE_CONTROL_OFFSET], hwif->hw.irq);
...
static int release_and_probe(int hwif)
{
	...
	ide_release_region(ide_hwifs[hwif].hw.io_ports[IDE_DATA_OFFSET],
			   (ide_hwifs[hwif].hw.io_ports[IDE_STATUS_OFFSET]-
			    ide_hwifs[hwif].hw.io_ports[IDE_DATA_OFFSET]));
	ide_release_region(ide_hwifs[hwif].hw.io_ports[IDE_CONTROL_OFFSET],1);
	
	/* Check for drives on first hwif */
...
int ideprobe_init (void)
{
	...
	if (empeg_hardwarerevision()>4) {
		...
		/* Check for drives. As the empeg boots in less time than the
		   drives can spin up in, we do this: we keep probing for 
		   drives, up to 20 attempts.

		   After each probe, we release the ports (etc) so we can
		   probe again without errors. If we find one drive, we try a
		   few more times for a second one, but basically we'll give
		   up on it if it doesn't appear within about a second of the
		   first one */
		do {
			/* Have we found drives on hwif 0? If not, probe for
			   them */
			...
			/* If we got any on hwif 0, then try hwif 1. Otherwise,
			   this is pointless */
			if (on_if0>0) {
				/* Check for drives on second hwif */
				on_if1=release_and_probe(1);
			}

			/* Did we get any drives? */
			if (on_if0==0 || on_if1==0) {
				/* No. Sleep and rescan in 0.25 seconds */
				int timeout=jiffies+(HZ/4);
				printk("No drives found, retrying...\n");
				while(jiffies<timeout);
				retries++;
			}
