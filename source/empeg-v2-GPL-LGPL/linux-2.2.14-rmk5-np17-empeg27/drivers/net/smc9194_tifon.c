...
{ 0x300,0, 0 };
#endif
/*  0x320, 0x340, 0x360, 0x380, 0x3A0, 0x3C0, 0x3E0, 0}; */


/* #define inb(adr)  *(volatile unsigned char  *)(PCMCIA_IOBASE_V+((adr)))
   #define inw(adr)  *(volatile unsigned short *)(PCMCIA_IOBASE_V+((adr)))

   #define outw(v,adr) *(volatile unsigned short *)(PCMCIA_IOBASE_V+((adr)))=v
   #define outb(v,adr) *(volatile unsigned char  *)(PCMCIA_IOBASE_V+((adr)))=v
*/
...
#define OSI_TECH 1
#if 1
static int pcmcia_attrbase=0;
static int pcmcia_manuid;
static int pcmcia_cardid;
#define PCMCIA_MANUID_OSITECH 0x140
...
static int pcmcia_enable_smc(void)
{
  short cor,chipid;
  int ioaddr=smc_portlist[0];


  ioaddr=smc_portlist[0];
  //	pcmcia_vddvpp( 5, 0);	
  udelay(1000);

  read_attr(); /* load base offset */

#ifdef CONFIG_SA1100_TIFON
  cor = *(volatile short *)(PCMCIA_ATTRBASE_V+pcmcia_attrbase);

  *(volatile short *)(PCMCIA_ATTRBASE_V+pcmcia_attrbase)=0x8080;
  *(volatile short *)(PCMCIA_ATTRBASE_V+pcmcia_attrbase)=0x8080;
  udelay(1000);
	
  *(volatile short *)(PCMCIA_ATTRBASE_V+pcmcia_attrbase)=0x4141;	
  *(volatile short *)(PCMCIA_ATTRBASE_V+pcmcia_attrbase)=0x4141;	
  udelay(10000);

  *(volatile short *)(PCMCIA_ATTRBASE_V+pcmcia_attrbase+2)=0;	
  udelay(1000);

  if ( pcmcia_manuid == PCMCIA_MANUID_OSITECH )
    {        
      set_bits(0x300, (ioaddr + OSITECH_AUI_PWR));
      set_bits(0x300, (ioaddr + OSITECH_RESET_ISR));
    }
#endif

#ifdef CONFIG_SA1100_EMPEG	
  /* Set our timings: fast register access. We only have to observe
     185ns cycle times, and this looks fine on the scope. Going down to
     0x162 gives timeouts, so this is obviously too fast. Preserve socket 0's
     timings, though. */
  MECR=(MECR&0xffff)|0x01630000;

  /* Reset chip */
  *(volatile unsigned char*)0xec008000=0xc0;
  *(volatile unsigned char*)0xec008000=0xc0;
  udelay(1000);

  /* Turn it on */
  *(volatile unsigned char*)0xec008000=0x41;  
  *(volatile unsigned char*)0xec008000=0x41;  
  udelay(10000);
  *(volatile unsigned char*)0xec008002=0;  
  udelay(1000);
#endif

  SMC_SELECT_BANK( 3 );
  chipid = inw( ioaddr+REVISION);
...
#endif
  printk("smc_init couldn't find card\n");
...
static int smc_probe( int ioaddr ) 
{
  ...
  if(( pcmcia_enable_smc() & 0xff00) != 0x3300 )
    return -ENODEV;	
...
#ifdef CONFIG_SA1100_EMPEG
  {
	  /* Our manufacturer OUI is 00:02:d7, our serial numbers are of the
	     form MMYYSSSSS, where MMYY is the batch date and SSSSS is the
	     unit serial number within that batch.

	     We take 17 bits of SSSSS, add 4 bits of MM and 3 bits of YY to
	     make the 24 bit MAC */
	  unsigned int empeg_id = get_empeg_id();
	  unsigned int mac_id = (empeg_id%100000)|
		  ((empeg_id/10000000)<<17)|
		  (((empeg_id/100000)%100)<<21);

	  /* Write it to the chip */
	  outw(0x0200,                                ioaddr + ADDR0 + 0 );
	  outw(((mac_id>>8)&0xff00)|0xd7,             ioaddr + ADDR0 + 2 );
	  outw(((mac_id&0xff)<<8)|((mac_id>>8)&0xff), ioaddr + ADDR0 + 4 );
  }
#else
  // HACK ! IP -addr fixed and wrong ??
  {
    volatile char *attrib_space=(volatile char *)PCMCIA_ATTRBASE_V;
    char buffer[256],*p; 
    int len;
    while ( *attrib_space != 0xFF ) {  /* Valid Tuple found */
      p=buffer;
	...
	*p++ = *attrib_space; 
	...
	if ( buffer[1]!= 8 ) break;
	if ( buffer[2]!= 4 ) break;
	if ( buffer[3]!= 6 ) break; /* 6 Byte */
                                        
	outw( (buffer[5])<<8|buffer[4], ioaddr + ADDR0 + 0 );
	outw( (buffer[7])<<8|buffer[6], ioaddr + ADDR0 + 2 );
	outw( (buffer[9])<<8|buffer[8], ioaddr + ADDR0 + 4 );
	break;
      }
    }
  }
...
#define CFG_16BIT 0x0080
...
#ifdef CONFIG_SA1100_EMPEG
  dev->irq = 7;
  GRER|=EMPEG_ETHERNETIRQ;
  GFER&=~EMPEG_ETHERNETIRQ;
  GEDR=EMPEG_ETHERNETIRQ;
#endif
  ...
  */
  ...
  /* SA1100 specific patch  Keith 19.5.99 */

  SMC_SELECT_BANK( 2 );
  outb( 0, ioaddr + INT_MASK );

  //irqval = request_irq(dev->irq, &smc_interrupt, 0, CARDNAME, dev);
  // uncomented by Stefan, replaced with line below
#if !defined(CONFIG_SA1100_EMPEG)
  INTPOL1 |= 0x02000000;
#endif
  irqval = request_irq(dev->irq, &smc_interrupt, SA_INTERRUPT, CARDNAME, dev);
  ...
  outb( SMC_INTERRUPT_MASK, ioaddr + INT_MASK );
  /*printk(CARDNAME": GPIO LVL 0x%08x\n", *(volatile unsigned long *)GPLR );*/
...
/*-------------------------------------------------------------
 ...
 --------------------------------------------------------------
*/
void smc_rcv(struct device *dev)
{
  struct smc_local *lp = (struct smc_local *)dev->priv;
  int 	ioaddr = dev->base_addr;
  int 	packet_number;
...
#define RS_ODDFRAME     0x1000
  ...
  packet_length -= (status & RS_ODDFRAME ? 5 : 6); 
#else
  packet_length -= 6; 
#endif	
  if ( !(status & RS_ERRORS ) ){			
