...
#define CONF_BASE_TIMEOUT	(HZ)	/* Initial timeout: 1 second */
#define CONF_RETRIES	 	5	/* 5 retries */
#define CONF_TIMEOUT_RANDOM	(HZ)	/* Maximum amount of randomization */
#define CONF_TIMEOUT_MULT	*5/4	/* Rate of timeout growth */
#define CONF_TIMEOUT_MAX	(HZ*30)	/* Maximum allowed timeout */
#define CONF_RETRIES_UPNP	10	/* 10 more retries */

/* this gives about 8.2 seconds timeout until UPnP is used as soon as available
   and 343 seconds until complete timeout (5 minutes 43 seconds)
*/
...
#if defined(CONFIG_IP_PNP_BOOTP) || defined(CONFIG_IP_PNP_RARP) || defined(CONFIG_IP_PNP_UPNP)
...
#ifdef CONFIG_IP_PNP_UPNP
u32 ic_upnp_arp_addr = INADDR_NONE;			/* Random LINKLOCAL address */
u32 ic_upnp_arp_reply = INADDR_NONE;			/* Address of last reply */
struct device *ic_upnp_dev = NULL;			/* Device we last sent on */
#endif
	...
	for (dev = dev_base; dev; dev = dev->next)
		if (user_dev_name[0] ? !strcmp(dev->name, user_dev_name) :
			...
			if (!(dev->flags & IFF_NOARP))
				able |= IC_RARP;
			if ((able & (IC_BOOTP | IC_RARP)) == (IC_BOOTP | IC_RARP))
				able |= IC_UPNP;
...
static void __init ic_bootp_init_ext(u8 *e)
{
	...
	*e++ = 83;
	*e++ = 99;
#ifdef CONFIG_IP_FUDGE_DHCPDISCOVER
...
static int ic_upnp_recv(struct sk_buff *skb, struct device *dev, struct packet_type *pt);

static struct packet_type upnp_packet_type __initdata = {
	__constant_htons(ETH_P_ARP),	/* UPnP is just a souped up ARP */
	NULL,				/* Listen to all devices */
	ic_upnp_recv,
	NULL,
	NULL
...
static inline void ic_upnp_init(void)
{
	dev_add_pack(&upnp_packet_type);
}

static inline void ic_upnp_cleanup(void)
{
	dev_remove_pack(&upnp_packet_type);
}

/*
 *  Process received UPnP packet.
 */
static int __init
ic_upnp_recv(struct sk_buff *skb, struct device *dev, struct packet_type *pt)
{
	...
	/* If we already know it's taken, drop this packet */
	if (ic_upnp_arp_addr == ic_upnp_arp_reply)
		goto drop;

	/* If this test doesn't pass, it's not IP, or we should ignore it anyway */
	if (arp->ar_hln != dev->addr_len || dev->type != ntohs(arp->ar_hrd))
		goto drop;

	/* If it's not an ARP request or reply, delete it. */
	...
	/* Discard packets which don't match the IP we're checking */
	if (ic_upnp_arp_addr != tip)
		goto drop;

	/* Damn - somebody has this address. Remember this */
	ic_upnp_arp_reply = ic_upnp_arp_addr;
	printk("IP-Config: Address %s in use by %02x:%02x:%02x:%02x:%02x:%02x\n",
	       in_ntoa(sip),
...
/*	Generate a random IP address from the LINKLOCAL range
 *	169.254/16 except:
 *	  x.  x.  0.  x
 *	  x.  x.255.  x
 *	  x.  x.  x.  0
...
static void ic_upnp_random_ip(void)
{
	u8 *addr = (u8 *) &ic_upnp_arp_addr;
	addr[0] = 169;
	addr[1] = 254;
	do {
		get_random_bytes(addr + 2, 2);
	} while(addr[2] == 0 || addr[2] == 255 || addr[3] == 0 || addr[3] == 255);
}


/*
 *  Send ARP request packet over all devices which allow ARP
 */
static void __init ic_upnp_send(void)
{
	struct ic_device *d;

	if (ic_upnp_arp_addr == ic_upnp_arp_reply) {
		/* somebody on our IP (or startup), find another one */
		ic_upnp_random_ip();
		
		printk("IP-Config: UPnP trying address %s\n", in_ntoa(ic_upnp_arp_addr));
	}

	/* Keep probing in either case */
		
	for (d=ic_first_dev; d; d=d->next)
		if (d->able & IC_UPNP) {
			struct device *dev = d->dev;
			arp_send(ARPOP_REQUEST, ETH_P_ARP, ic_upnp_arp_addr, dev, 0, NULL,
				 dev->dev_addr, dev->dev_addr);
			ic_upnp_dev = dev;
		}
}
...
/*
 * Promiscuous ARP reply - update caches, reduce chance of collision
 */
static void __init ic_upnp_solicit(void)
{
	arp_send(ARPOP_REPLY, ETH_P_ARP, ic_upnp_arp_addr,
		 ic_upnp_dev, ic_upnp_arp_addr,
		 NULL, ic_upnp_dev->dev_addr, ic_upnp_dev->dev_addr);
}
...
static int __init ic_dynamic(void)
{
	...
	int do_upnp = ic_proto_have_if & IC_UPNP;
...
#ifdef CONFIG_IP_PNP_UPNP
	if ((ic_proto_enabled ^ ic_proto_have_if) & IC_UPNP)
		printk(KERN_ERR "UPNP: No suitable device found.\n");
#endif
...
#ifdef CONFIG_IP_PNP_UPNP
		/* Advertise ourselves */
		if (do_upnp && ic_upnp_arp_addr != ic_upnp_arp_reply)
			ic_upnp_solicit();
#endif

		if (! --retries) {
			if (do_upnp) {
				printk(" %s%s%s timeout!\n",
				       do_bootp ? "BOOTP" : "",
				       do_bootp && do_rarp ? " / " : "",
				       do_rarp ? "RARP" : "");
			}
...
#ifdef CONFIG_IP_PNP_UPNP
	if (ic_upnp_arp_addr != ic_upnp_arp_reply) {
		printk("IP-Config: UPnP address %s is free\n",
		       in_ntoa(ic_upnp_arp_addr));

		/* Which device do we bind to? */

		/* Pick the one we used last for now... */
		
		ic_dev = ic_upnp_dev;
		ic_myaddr = ic_upnp_arp_addr;
