 ...
 *
 * (:Empeg Source Release 1.131 13-Mar-2003 18:15 rob:)
 */
...
class ProtocolClient::RequestProgress: public RequestObserver 
    {
	ProtocolClient *client;
	Activity activity;

    public:
	RequestProgress(ProtocolClient *c, Activity a) : client(c), activity(a) { }
	virtual void ProgressAction(Request *req, int stage, int stage_maximum, 
				    int current, int maximum, char *string);
	virtual ~RequestProgress() { }
    };
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::StatFid(fid=0x%x, psize=%p)\n", fid, psize);
    ASSERT(psize);
    STATUS result;
    ...
    TRACEC(TRACE_PROTOCOL,"Starting to do StatFID\n");
    do
    {
    ...
    if ( SUCCEEDED(result) )
	TRACEC(TRACE_PROTOCOL, "ProtocolClient::StatFid() = %d\n", *psize);
    else
	TRACEC(TRACE_PROTOCOL, "ProtocolClient::StatFid: error 0x%x\n\n",
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::ReadFidPartial(0x%x, %p, %d, %p)\n", fid,
	...
	if ( FAILED(result) )
	{
	    TRACEC(TRACE_PROTOCOL, "ReadFid(fid=0x%x, offset=0x%x, size=0x%x) failed with error 0x%x, retries=%d\n\n",
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::ReadFidPartial() returns 0x%x\n",
	      PrintableStatus(result));
    return result;
}
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::ReadFidToMemory2(0x%x)\n", fid);
    ...
    *ppfm = f;
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::ReadFidToMemory() = %d\n",
	      PrintableStatus(result));
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::ReadFidToMemory(0x%x, %p, %p)\n", fid,
		      ppdest, psize);
    STATUS result;
    int size;
    Request r(connection);
    ...
    if ( FAILED(result) )
    {
	TRACEC(TRACE_PROTOCOL, "ProtocolClient::ReadFidToMemory()\' = 0x%x\n",
		  PrintableStatus(result));
	return result;
    }
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::PrepareFid(fid=0x%x, size=%d)\n",
		      fid, size);
    Request r(connection);
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::PrepareFid() = 0x%x\n", PrintableStatus(result));
    return result;
}
...
STATUS ProtocolClient::WriteFidFromMemory(FILEID fid, const BYTE *psource,
					  int size)
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::WriteFidFromMemory(fid=0x%x, psource=%p, size=%d)\n",
		      fid, psource, size);
    Request r(connection);
    ...
    if (FAILED(result))
    {
	TRACEC(TRACE_PROTOCOL, "ProtocolClient::WriteFidFromMemory() = %x\n",
		  PrintableStatus(result));
	return result;
    }
	...
	do
	{
	    ...
	    if (FAILED(result))
	    {
		TRACEC(TRACE_PROTOCOL, "WriteFID(fid=0x%x, offset=0x%x, size=0x%x) failed with error %d\n\n",
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::WriteFidFromMemory() = %d\n",
	      PrintableStatus(result));
    return result;
}
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::WriteFidFromFile(fid=0x%x, filename=%s)\n",
		      fid, filename);
	...
	if (FAILED(result))
	{
	    TRACEC(TRACE_PROTOCOL, "Failed to PrepareFID(0x%x, 0x%x) - result=0x%x\n",
    ...
    if (FAILED(result))
    {
	TRACEC(TRACE_PROTOCOL, "ProtocolClient::WriteFidFromFile(A)\' = %d\n",
		  PrintableStatus(result));
	return result;
    }
    ...
    if (fc)
    {
        TRACEC(TRACE_PROTOCOL, "Attempting fastwrite(%u)\n\n", size);
        ...
        if (FAILED(result))
        {
            TRACEC(TRACE_PROTOCOL, "Writing fast-protocol header failed\n\n");
	    TRACEC(TRACE_PROTOCOL, "ProtocolClient::WriteFidFromFile(H) = %x\n",
		      PrintableStatus(result));
            return result;
        }
        ...
        if ( remain > 0 )
        {
            TRACEC(TRACE_PROTOCOL, "Fastwrite has %u bytes remaining\n\n", remain);
        }
    }
    else
    {
    ...
    TRACEC(TRACE_PROTOCOL, "WriteFID(%d bytes) took %ldms (%d.%02dK/s)\n\n", size, elapsedTime,
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::WriteFidFromFile(C) = 0x%x\n",
	      PrintableStatus(result));
    return result;
}
...
STATUS ProtocolClient::WriteFidPartial(FILEID fid, const BYTE *psource,
				       int pos, int size)
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::WriteFidPartial(fid=0x%x, psource=%p, pos=%d, size=%d)\n",
	...
	do
	{
	    ...
	    if (FAILED(result))
	    {
		TRACEC(TRACE_PROTOCOL, "WriteFID(fid=0x%x, offset=0x%x, size=0x%x) failed with error %x\n\n",
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::WriteFidPartial() = %x\n",
	      PrintableStatus(result));
    return result;
}
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::FreeFidMemory(ppdest=%p)\n",ppdest);
    TRACEC(TRACE_PROTOCOL, "*ppdest=%p\n", *ppdest);
    ASSERT(ppdest);
    EFREE(*ppdest);
    *ppdest = NULL;
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::FreeFidMemory()\n");
}

STATUS ProtocolClient::CheckDatabaseAvailability()
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::CheckDatabaseAvailability()\n");
    STATUS result;
    int size = 0;
    ...
    if (SUCCEEDED(result))
    {
	TRACEC(TRACE_PROTOCOL, "Tag index size is %d bytes\n\n", size);

	if (size < 16)
	{
	    TRACEC(TRACE_PROTOCOL, "Tag database is suspiciously small. Failing\n");
    ...
    {
	TRACEC(TRACE_PROTOCOL, "Tag index stat failed: %d\n", PrintableStatus(result));
    }
    if (SUCCEEDED(result))
    {
	...
	if (SUCCEEDED(result))
	{
	    TRACEC(TRACE_PROTOCOL, "Static database size is %d bytes\n\n", size);
            // A completely empty database is around ten bytes.
	    if (size < 10)
	    {
		TRACEC(TRACE_PROTOCOL, "Static database is suspiciously small. Failing.\n");
		// Pretend it doesn't exist
		result = MakeErrnoStatus(LINUX_ENOENT);
	    }
	}
	...
	{
	    TRACEC(TRACE_PROTOCOL, "Static database stat failed: %d\n\n", PrintableStatus(result));
	}
    }
    if (SUCCEEDED(result))
    {
	...
	if (SUCCEEDED(result))
	{
	    TRACEC(TRACE_PROTOCOL, "Playlist database size is %d bytes\n\n", size);
	}
	else
	{
	    TRACEC(TRACE_PROTOCOL, "Playlist database stat failed: %d\n\n", PrintableStatus(result));
	}
    }
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::CheckDatabaseAvailability() = %d\n", PrintableStatus(result));
    return result;
}
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::RetrieveTagIndex(db_observer=%p)\n", db_observer);
    STATUS result;
    BYTE *tag_buffer;
    int tag_buffer_size;
	...
	if (tag_buffer_size <= 16)
	{
	    ...
	    TRACEC(TRACE_PROTOCOL, "Tags file is suspiciously short (%d bytes), failing gracefully.\n", tag_buffer_size);
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::RetrieveTagIndex() = %d\n", PrintableStatus(result));
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::DeleteDatabases()\n");
    STATUS result;
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::DeleteDatabases() = %d\n", PrintableStatus(result));
    return result;
}
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::RetrieveDatabases(db_observer=%p)\n", db_observer);
    ...
    if (FAILED(result))
    {
	TRACEC(TRACE_PROTOCOL, "ProtocolClient::RetrieveDatabases()\' = %d\n", PrintableStatus(result));
	return result;
    }
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::IsUnitConnected()\n");
    Request r(connection);
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::IsUnitConnected()=%d\n", result);
    return result;
}

STATUS ProtocolClient::WaitForUnitConnected(int timeout)
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::WaitForUnitConnected(timeout=%d)\n", timeout);
...
//		TRACE("reconnect succeeded\n");
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::WaitForUnitConnected()=%d\n", PrintableStatus(result));
    return result;
}

STATUS ProtocolClient::EnableWrite(bool b)
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::EnableWrite(b=%d)\n", b);
    Request r(connection);
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::EnableWrite()=%d\n", PrintableStatus(result));
    return result;
}

STATUS ProtocolClient::RebuildPlayerDatabase(int options)
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::RebuildPlayerDatabase(options=%d)\n", options);
    Request r(connection);
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::RebuildPlayerDatabase()=%d\n", PrintableStatus(result));
    return result;
}

STATUS ProtocolClient::DeleteFid(FILEID fid, int mask)
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::DeleteFid(fid=%d, mask=0x%x)\n", fid, mask);
    Request r(connection);
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::DeleteFid()=%d\n", PrintableStatus(result));
    return result;
}
...
STATUS ProtocolClient::GetDiskInfo(DiskSpaceInfo *info, int count)
{
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::GetDiskInfo(info=%p, count=%d)\n", info, count);
    ...
    TRACEC(TRACE_PROTOCOL, "Disk 0: size=%ld, space=%ld, block_size=%ld\n",
	     (long)info[0].size, (long)info[0].space, (long)info[0].block_size);
    TRACEC(TRACE_PROTOCOL, "Disk 1: size=%ld, space=%ld, block_size=%ld\n",
	     (long)info[1].size, (long)info[1].space, (long)info[1].block_size);

    TRACEC(TRACE_PROTOCOL, "ProtocolClient::GetDiskInfo() = %d\n", PrintableStatus(result));
    return result;
}

STATUS ProtocolClient::Fsck(const char *drive, unsigned int *pflags)
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::Fsck(drive='%s')\n", drive);
    Request r(connection);
    ...
    *pflags = 0;
	...
	if (fsck_flags & 1)
	{
	    TRACE("FSCK corrected some errors on drive %s\n", drive);
	}
	if (fsck_flags & 2)
	{
	    TRACE("FSCK thinks the system should be rebooted on drive %s\n", drive);
	    bad = true;
	}
	if (fsck_flags & 4)
	{
	    TRACE("FSCK left stuff uncorrected on drive %s\n", drive);
	    bad = true;
	}
	if (fsck_flags & 8)
	{
	    TRACE("FSCK reported operational error on drive %s\n", drive);
	    bad = true;
	}
	if (fsck_flags & 16)
	{
	    TRACE("FSCK report usage or syntax error on drive %s\n", drive);
	    bad = true;
	}
	if (fsck_flags & 128)
	{
	    TRACE("FSCK reported shared library error on drive %s\n", drive);
	    bad = true;
	}
	if (fsck_flags & 512)
	{
	    TRACE("FSCK reported disk is not present on drive %s\n", drive);
	}
	...
	*pflags = fsck_flags;
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::Fsck()=%x\n", PrintableStatus(result));
    return result;
}

STATUS ProtocolClient::CheckMedia( unsigned *pfsck_flags )
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::CheckMedia()\n");
    STATUS result;
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::CheckMedia()=%d\n", PrintableStatus(result));
    return result;
}
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::SendCommand(command=%d, p0=%d, p1=%d, p2=%p\n", command, parameter0, parameter1, parameter2);
    Request r(connection);
    STATUS result;
    int retries = 0;
    do
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::SendCommand()=%d\n", PrintableStatus(result));
    return result;
}
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::InternalRestart(param=%d, wait_for_completion=%d)\n", param, wait_for_completion);

    STATUS result = SendCommand(COM_RESTART, param);
    if (SUCCEEDED(result))
    {
	TRACEC(TRACE_PROTOCOL, "Restart command sent, waiting for player to begin responding\n\n");
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::InternalRestart()=%d\n", PrintableStatus(result));
    return result;
}

STATUS ProtocolClient::LockUI(bool lock)
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::LockUI(%d)\n", lock);
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::SetSlumber(%d)\n", slumber);
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::PlayFid(0x%x)\n", fid);
    STATUS result = SendCommand(COM_PLAYFID, fid, mode);
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::PlayFid()=%d\n", PrintableStatus(result));
    return result;
}

STATUS ProtocolClient::PlayFids(const vecFids &fids, int mode)
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::PlayFids(0x%x)\n",
	   (unsigned) fids.size());
    STATUS result = PlayFidList(fids, mode);
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::PlayFids()=%d\n", PrintableStatus(result));
    return result;
}

STATUS ProtocolClient::SetPlayState(int pp)
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::SetPlayState(0x%x)\n", pp);
    STATUS result = SendCommand(COM_PLAYSTATE, pp);
    //printf("Result=%d\n", result);
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::SetPlayState()=%d\n", PrintableStatus(result));
    return result;
}

STATUS ProtocolClient::GrabScreen(unsigned char *screen)
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::GrabScreen(screen=%p)\n", screen);
    Request r(connection);

    // Don't bother to retry this :-)
    STATUS result = r.GrabScreen(0,screen);
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::GrabScreen()=%x\n", PrintableStatus(result));
    return result;
}
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::InitiateSession\n");
    Request r(connection);
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::SessionHeartbeat\n");
    Request r(connection);
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::TerminateSession\n");
    Request r(connection);
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::NotifySyncComplete\n");
    Request r(connection);
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::GetPlayerType\n");
    char *buffer = NULL;
    int length;
	...
	*type = std::string(buffer, length - 1);
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::GetPlayerType()=%x\n", PrintableStatus(result));
    return result;
}

STATUS ProtocolClient::GetVersionInfo(PlayerVersionInfo *info)
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::GetVersionInfo(info=%p)\n", info);
    ASSERT(info);
    char *buffer = NULL;
    int length;
	...
	TRACEC(TRACE_PROTOCOL, "Connected player version information:\n");
	TRACEC(TRACE_PROTOCOL, "   Version:  %s\n", info->version);
	TRACEC(TRACE_PROTOCOL, "   Beta:     %s\n", info->beta);
	TRACEC(TRACE_PROTOCOL, "   Protocol: %d\n", info->protocol);
    }
    FreeFidMemory(reinterpret_cast<BYTE **>(&buffer));
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::GetVersionInfo()=%x\n", PrintableStatus(result));
    return result;
}

STATUS ProtocolClient::GetProtocolVersion(short *major, short *minor)
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::GetProtocolVersion(major=%p, minor=%p)\n",
	      major, minor);
	...
	*minor = m_protocol_version_minor;
    ...
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::GetProtocolVersion()=%d\n",
	      PrintableStatus(result));
    return result;
}
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::GetPlayerConfiguration(s=%p)\n", s);
    BYTE *data;
    int size;
	...
	TRACEC(TRACE_PROTOCOL, "The player configuration file config.ini didn't exist, nomatter.\n");
	s->assign("");
	result = S_OK;
    }

    TRACEC(TRACE_PROTOCOL, "ProtocolClient::GetPlayerConfiguration() = %d\n",
	      PrintableStatus(result));
    return result;
}
...
{
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::SetPlayerConfiguration()\n");
    STATUS result = WriteFidFromMemory(FID_CONFIGFILE,
				   reinterpret_cast<const BYTE *>(s.data()),
				       s.length());
    TRACEC(TRACE_PROTOCOL, "ProtocolClient::SetPlayerConfiguration() = %d\n",
	      PrintableStatus(result));
    return result;
}
...
void ProtocolClient::ErrorAction(STATUS result, int retries)
{
	...
	{
#ifdef WIN32
	    TRACEC(TRACE_PROTOCOL, "Sleeping for a while due to excessive retries.\n");
	    ::Sleep(1000);
#else
	    sleep(1);
#endif
...
STATUS ProtocolClient::GetPlayerIdentity(PlayerIdentity *id)
{
	...
	while (true)
	{
		...
		if (p != line_start)
		{
		    ...
		    TRACEC(TRACE_PROTOCOL, "Got a line: %s\n", line.c_str());
			...
			TRACEC(TRACE_PROTOCOL, "Field=%s, value=%s\n", field.c_str(), value.c_str());
