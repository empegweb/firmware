...
/** We've been given an explicit address to try, so we'll just hook up
 * a connection to it, and try connecting...
 */
STATUS UnicastNetworkEmpegDiscoverer::Discover(unsigned /*timeout_ms*/,
						bool * /*pbContinue*/,
						bool * /*pbIgnoreFailure*/)
{
    // We've been given an explicit address to try, so we'll just hook
    // up a connection to it, and try connecting...
	...
	if (client.IsUnitConnected())
	{
		...
		if (!dcf.GetStringValue("options", "name", &name))
		    name = GetDefaultName(playerType);
	    }
