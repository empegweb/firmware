 ...
 * empeg ltd.
 *
 * (:Empeg Source Release 1.40 13-Mar-2003 18:15 rob:)
 */
...
public:
	...
	{
	    TRACE_ERROR("seq=%d guard1(%p)=0x%08lx - called by %p %s:%d\n",
		  seq, &guard1, guard1, called_by, file, line);
	    return false;
	}
	if (guard2 != GUARD2_VALUE)
	{
	    TRACE_ERROR("seq=%d guard2=0x%08lx - called by %p %s:%d\n",
		  seq, guard2, called_by, file, line);
	    return false;
	}
	if (guard3 != GUARD3_VALUE)
	{
	    TRACE_ERROR("seq=%d guard3=0x%08lx - called by %p %s:%d\n",
		  seq, guard3, called_by, file, line);
	    return false;
	}
	if (guard4 != GUARD4_VALUE)
	{
	    TRACE_ERROR("seq=%d guard4=0x%08lx - called by %p %s:%d\n",
		  seq, guard4, called_by, file, line);
	    return false;
	}
	return true;
...
{
    if(!s) TRACE_WARN("Attempt to allocate zero bytes %s:%d\n", szFile, nLine);
...
void *my_alloc(size_t s, AllocType type, const char *szFile, int nLine, void *caller) THROW_NONE
{
    ...
    if(!s) TRACE_WARN("Attempt to allocate zero bytes %s:%d, called by %p\n",
		szFile, nLine, caller);
...
void dumpleaks()
{
	...
	TRACE_WARN("*LEAK* Memory leak dump...\n");
	while (p)
	{
//	    char buffer[64];
...
//	    char *o = buffer;
		...
		default:
		    TRACE_WARN("Unknown reason code: %d\n", p->nLine);
		    reason = "????";
		    break;
		}

		if (reason)
		    TRACE_WARN("*LEAK* Object %d at %p size %u (%s) (caller=%p)\n",
			 p->nSeq, p+1, p->size, reason, p->caller);
	    }
	    else
	    {
		TRACE_WARN("*LEAK* Object %d at %p size %u by %s:%d (caller=%p)\n",
	...
	TRACE_WARN("-----------------------------------------\n");
	TRACE_WARN("Leak dump complete (%u harmless ignored).\n", harmless_count);
    }
    else
    {
...
void dumpfds()
{
	...
	if (r < 0)
	{
	    TRACE_WARN("readlink failed, errno = %d\n", errno);
	    filename[0] = '\0';
	}
	else
	    filename[r] = '\0';
...
void *empeg_realloc(void *pv, size_t s, const char *file, int line, void *caller)
{
    ...
    if(!s) TRACE_WARN("Attempt to reallocate zero bytes %s:%d, called by %p\n",
		file, line, caller);
...
void *alloc_throw(size_t s)
{
    ...
    TRACE_ERROR("Memory allocation failed!\n");
...
#ifdef ECOS
// This takes up slightly less space than the default new/delete

#include <pkgconf/kernel.h>   // Configuration header
#include <cyg/kernel/kapi.h>
#include <cyg/infra/diag.h>
#include <cyg/hal/hal_granite.h>
#include <stdlib.h>
...
{
    // You can pass NULL to delete in C++
    if(p != NULL)
	free(p);
}
