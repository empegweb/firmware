...
STATUS FileStream::Open(const tstring &file_name,
			unsigned flags, unsigned mode)
{
    ...
    {
    case 0:
	TRACE_WARN("Can't open a file without read/write flag\n");
	return E_NOTIMPL;
    ...
    m_handle = empeg_open(file_name.c_str(), openflags, mode);
    if(m_handle < 0)
    {
	TRACE_WARN("Failed to open file \"%s\": errno=%d \'%s\'\n", file_name.c_str(), errno, strerror(errno));
	return MakeErrnoStatus();
    }
...
STATUS FileStream::Close()
{
    ...
    if(close(m_handle))
    {
	TRACE_WARN("Failed to close file \"%s\": %m\n", m_file_name.c_str());
	return MakeErrnoStatus();
    }
    m_handle = -1;
    m_is_open = false;
			...
			unsigned *bytes_read)
{
    ...
    if(n < 0)
    {
	TRACE_WARN("Failed to read from file \"%s\": %m\n", m_file_name.c_str());
	return MakeErrnoStatus();
    }
...
#if 0
STATUS FileStream::ReadOffset(unsigned int offset, void *buffer,
			      unsigned int bytes, unsigned int *bytesread)
{
    ASSERT(m_is_open); 
    
    int n = pread(m_handle, buffer, bytes, offset);
			 ...
			 unsigned *bytes_written)
{
    ...
    if(n < 0)
    {
	TRACE_WARN("Failed to write to file \"%s\": %m\n", m_file_name.c_str());
	return MakeErrnoStatus();
    }
    *bytes_written = n;
    m_current_pos += n;
...
STATUS FileStream::Create(const TCHAR *filename, unsigned int flags, FileStream **result )
{
