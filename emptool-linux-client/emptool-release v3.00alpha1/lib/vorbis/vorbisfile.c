...
static void _make_decode_ready(OggVorbis_File *vf){
  if(vf->ready_state!=OV_STREAMSET)return;
  if(vf->seekable){
  ...
  while(1){
    ...
    */
    ...
    if(vf->ready_state!=OV_INITSET){ 
      ...
      if(vf->ready_state<OV_STREAMSET){
	if(vf->seekable){
...
static int _ov_open1(void *f,OggVorbis_File *vf,char *initial,
  ...
  if((ret=_fetch_headers(vf,vf->vi,vf->vc,&vf->current_serialno,NULL))<0){
    vf->datasource=NULL;
    ov_clear(vf);
  }else if(vf->ready_state < OV_PARTOPEN)
    vf->ready_state=OV_PARTOPEN;
  return(ret);
}
...
int ov_test_open(OggVorbis_File *vf){
  if(vf->ready_state!=OV_PARTOPEN)return(OV_EINVAL);
  return _ov_open2(vf);
}
...
long ov_bitrate(OggVorbis_File *vf,int i){
  if(vf->ready_state<OV_OPENED)return(OV_EINVAL);
