 ...
 *
 * (:Empeg Source Release 1.34 01-Apr-2003 18:52 rob:)
 */
...
STATUS ID3TagExtractor::Create(const std::string &filename, TagExtractor **pp)
{
...
STATUS ID3TagExtractor::ExtractTags(SeekableStream *pStm, TagExtractorObserver *pObserver)
{
    ...
    collect.OnTextFrame("0TIT", util::TruncateFilename(m_filename).c_str());
    ExtractV1Tag(pStm, &collect);
    unsigned offset = 0;
    ExtractV2Tag(pStm, &collect, &offset, pObserver);
    ...
    for (FrameMapping *m = frameMapping; m->tagName; ++m)
    {
	for (const char **q = m->frameIDs; *q; ++q)
	{
	    const char *p = *q;
...
//    printf("  %s : %s\n", frameID.c_str(), frameData.c_str());
    ...
    if (frameID == "TCON")
    {
	std::string genre(frameData);

	int genre_num = -1;
	if (sscanf(genre.c_str(), "(%d)", &genre_num) == 1)
	    genre = GenreList::Lookup(genre_num);
