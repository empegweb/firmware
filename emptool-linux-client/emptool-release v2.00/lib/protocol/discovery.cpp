...
std::string EmpegDiscoverer::GetEmpegName(Connection *connection, const std::string &defaultName)
{
	...
	if (SUCCEEDED(client.GetPlayerConfiguration(&config)))
	{
	    ...
	    s = dcf.GetStringValue("options", "name", s);
	}
...
HRESULT CompoundEmpegDiscoverer::Discover(unsigned timeout_ms, bool *pbContinue, bool *pbIgnoreFailure)
{
	...
	{
	    EmpegDiscoverer *d = *i;
	    HRESULT hr = d->Discover(individual_timeout_ms, pbContinue);
