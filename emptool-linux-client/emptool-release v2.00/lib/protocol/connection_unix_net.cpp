...
STATUS OutSocketConnection::FillBuffer(unsigned char *buffer, int buffer_size, int timeout_ms, DWORD & bytes_read)
{
	...
	if (n < 0)
	{
	    WARN("poll on socket failed: %s (%d)\n", strerror(errno), errno);
	    return MakeErrnoStatus();
	}
	if (!n)
	{
	    WARN("poll on socket timed out (%dms)\n", timeout_ms);
    ...
    //WARN("otc::fb read returned %d\n", n );

    if (n < 0)
    {
	WARN("read on socket failed: %s (%d)\n", strerror(errno), errno);
...
int InSocketConnection::OpenPipeIfNecessary(int timeout_ms)
{
	...
	if (n < 0)
	{
	    WARN("Poll on accept socket failed.\n");
	    return n;
	}
	else if (n == 0)
	{
