...
STATUS IPAddress::FromHostName(const std::string &hostname)
{
	...
	if (!h)
		...
		// This is a problem with the buffer size - better programming required!
                WARN("IPAddress::FromHostName application error - make the buffer bigger!\n");
                return MakeErrnoStatus(ENOMEM);
	    case HOST_NOT_FOUND:
		WARN("IPAddress::FromHostName host '%s' not found\n", hostname.c_str());
	...
	if (!h)
	{
	    ERROR("gethostbyname_r failed in a wierd way - h_errno=%d\n", host_errno);
