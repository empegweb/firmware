...
//    printf("parse_ls: cwdstack = \"%s\"\n", BranchStackToString(cwdstack).c_str());
	...
	while(it != direntry_list.end()) {
		...
		if (FAILED(status)) {
		    ...
		    node->SetTag( db.GetTagNumber("artist"),
				  teo["artist"].c_str() );
		    node->SetTag( db.GetTagNumber("comment"),
				  teo["comment"].c_str() );
		    node->SetTag( db.GetTagNumber("genre"),
				  teo["genre"].c_str() );
		    node->SetTag( db.GetTagNumber("source"),
				  teo["source"].c_str() );
		    node->SetTag( db.GetTagNumber("year"),
				  teo["year"].c_str() );
		    node->SetTag( db.GetTagNumber("codec"),
				  teo["codec"].c_str() );
		    node->SetTag( db.GetTagNumber("bitrate"),
				  teo["bitrate"].c_str() );
		    node->SetTag( db.GetTagNumber("offset"),
				  teo["offset"].c_str() );
		    node->SetTag( db.GetTagNumber("samplerate"),
				  teo["samplerate"].c_str() );
		    node->SetTag( db.GetTagNumber("duration"),
				  teo["duration"].c_str() );
		    node->SetTag( db.GetTagNumber("tracknr"),
				  teo["tracknr"].c_str() );
		}
